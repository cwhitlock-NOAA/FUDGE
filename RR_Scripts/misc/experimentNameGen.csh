...
<table id="exp_info">
<thead>
<tr align="left"><th width=25%>Name</th><th width=10%>DS Method   </th><th width=5%>Series    </th><th width=12%>GCM    </th><th width=10%>Epoch    </th><th width=10%>Obs    </th><th width=10%>Kfold    </th><th width=10%></tr>
</thead>
EOF
...
foreach dsMethod (CDFt BCQM EDQM)
 foreach varID (tx tn pr)
  if ($varID == "tx" || $varID == "tn") set suffix="at"
  if ($varID == "pr") set suffix="ap"
  foreach gcm (CCSM4 MIROC5 "MPI-ESM-LR")
   ...
   foreach epoch (historical rcp45 rcp85)
    ...
    set expName = "${regionID}${varID}${platformID}-${dsMethod}-${expSeries}${gcmID}${epochID}${suffix}${obsID}${kfoldID}"
...
\cp RR_exp.html /home/esd/internal_html
