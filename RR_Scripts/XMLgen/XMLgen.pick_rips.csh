...
source /usr/local/Modules/default/init/csh
  ...
  echo "####################################################################################################### "
  echo " #####"
  echo " ##### IMPORTANT NOTE: This version REQUIRES user to specify the RIP for the future predictor GCM. "
  echo " ##### It is intended mainly for the runs using a historical scenario for the 'future predictor' GCM."
  echo " #####"
  echo "####################################################################################################### "
  echo " "
  sleep 5
...
#============================================================
  ...
  echo " -  While predetermined preferred realizations for CCSM4, MIROC5, MPI-ESM-LR GCMS will be used for "
  echo "      'historical predictor' for RR, the user must specify the realization to be used for 'future predictor'."
  echo "      THIS IS ESPECIALLY DESIRED IF 'future predictor' is another historical realization."
  echo " -  User must specify the realization of the GCM to use. "
  echo " "
  echo "======================================================================================================= "
...
# get RIP for historical predictor
...
#  set futPredRIP = $myRip
  ...
  echo "    ========================================== "
  echo "    SELECTION OF FUTURE PREDICTOR REALIZATION. "
  echo "    ========================================== "
  set dinfo = "$futPredDataset 'FUTURE Predictor', you need to specify a RIP; NOTE: Historical Predictor = $histPredDataset $futPredEpoch $histPredRIP"
  echo " " 

  unset expInfo
  set expInfo = (`grep "$futPredDataset" $xmlGenDir/surface_variable_gcm_lookup_table | grep "$futPredEpoch"`)
      if ($status != 0) exit 1 
  set nexp = $expInfo[3]
  set varvals = ()
  set versions = ()
  ...
  set kver = 5
  while ($k <= $nexp)
    set varvals = ($varvals $expInfo[$krip])
    set versions = ($versions $expInfo[$kver])
    @ krip  = $krip + 2
    @ kver  = $kver + 2
    @ k = $k + 1
  end
  echo " "
  echo "   NOTE: Historical Predictor is realization  $histPredRIP ."
  echo " "
  sleep 4
  source $QueryVals
  set futPredRIP = "$varvals[$kvar]"
  set futPredVersion = "$versions[$kvar]"
...
# get version number from lookup table
  ...
  if ($futPredVs != $futPredVersion) exit 1
#......keep above?.....
